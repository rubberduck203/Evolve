...
/***********************************************************************
 * BINARY VERSION 7 READ/WRITE operations.
 *
 * This module reads/writes objects to files.
 *
 * What's new from version 6 to 7?
 * ===============================
...
#define EVOLVE_BINARY_MAGIC	"EVOLVE_BINARY_FORMAT:V07     "
 ...
 *
 */
int Universe_WriteBinary7(UNIVERSE *u, const char *filename, char *errbuf)
{
	FILE *fp;
	bool success;
...
UNIVERSE *Universe_ReadBinary7(const char *filename, char *errbuf)
{
	UNIVERSE *u;
	FILE *fp;
	bool success;
...
int Universe_Migrate_7to8(const char *filename, char *errbuf)
{
	UNIVERSE *u;
	int n;
	...
	u = Universe_ReadBinary7(filename, errbuf);
	if( u == NULL ) {
	...
	/*
	 * New option has been added, upgrade so the default exists.
	 *
	 */
	u->kfmo->max_code_blocks = 100;

	n = Universe_WriteBinary8(u, filename, errbuf);
	if( !n ) {
		sprintf(errbuf, "%s", errbuf);
		return 0;
	}
