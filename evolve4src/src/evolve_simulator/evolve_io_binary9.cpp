...
/***********************************************************************
 * BINARY VERSION 9 READ/WRITE operations.
 *
 * This module reads/writes objects to files.
 *
 * What's new from version 8 to 9?
 * ===============================
 *	* radio active dye marker
 *	* Added new entity "player" to support game mode
 *	* Added new mutation option Max Apply.
 *
 */
#include "stdafx.h"

/*                               0123456789012345678901234567890 */
#define EVOLVE_BINARY_MAGIC	"EVOLVE_BINARY_FORMAT:V09     "
...
static int fwrite_KFORTH_MUTATE_OPTIONS(KFORTH_MUTATE_OPTIONS *kfmo, FILE *fp)
{
	...
	n = evbio_fwrite_int(&kfmo->max_apply, fp);
	if( n != 1 )
		return n;
...
static int fread_KFORTH_MUTATE_OPTIONS(KFORTH_MUTATE_OPTIONS *kfmo, FILE *fp)
{
	...
	n = evbio_fread_int(&kfmo->max_apply, fp);
	if( n != 1 )
		return n;
...
static int fwrite_ORGANISM(ORGANISM *o, FILE *fp)
{
	...
	n = evbio_fwrite_int(&o->oflags, fp);
	if( n != 1 )
		return n;
...
static int fwrite_SPORE(SPORE *spore, FILE *fp)
{
	...
	n = evbio_fwrite_int(&spore->sflags, fp);
	if( n != 1 )
		return n;
...
static int fread_SPORE(SPORE *spore, FILE *fp)
{
	...
	n = evbio_fread_int(&spore->sflags, fp);
	if( n != 1 )
		return n;
...
UNIVERSE *Universe_ReadBinary9(const char *filename, char *errbuf)
{
	UNIVERSE *u;
	FILE *fp;
	bool success;
